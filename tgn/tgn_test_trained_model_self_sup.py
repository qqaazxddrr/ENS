...
def args_parser():
    ...
    parser.add_argument('--cl', action='store_true',
                        help='Whether to run the curriculum learning mode')
...
def main():
    ...
    if not CURRICULUM_LEARNING:
        ...
        if args.model == 'tgn':
            prefix = 'tgn_attn_cl_{}'.format(PACING)
            log_prefix = 'TGN_CL_{}'.format(PACING)
        elif args.model == 'jodie':
            prefix = 'jodie_rnn_cl_{}'.format(PACING)
            log_prefix = 'JODIE_CL_{}'.format(PACING)
        elif args.model == 'dyrep':
            prefix = 'dyrep_rnn_cl_{}'.format(PACING)
            log_prefix = 'DyRep_CL_{}'.format(PACING)
        else:
            print('Not a valid model name!')
            prefix = ''
            log_prefix = ''
    ...
    # setup logger
    if not CURRICULUM_LEARNING:
        LOG_FILENAME = './log/neg_sample/{}_{}_{}_{}.log'.format(time_value.strftime('%Y_%m_%d'),log_prefix, NEG_SAMPLE, DATA)
    else:
        LOG_FILENAME = './log/neg_sample/{}_{}_{}_{}_CL.log'.format(time_value.strftime('%Y_%m_%d'),log_prefix, NEG_SAMPLE, DATA)
    ...
    from utils.log_summerize import summerize
