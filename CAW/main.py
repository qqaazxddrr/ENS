...
logger, get_checkpoint_path, best_model_path, log_file_path = set_up_logger(args, sys_argv)

# Load data and sanity check
g_df = pd.read_csv('./data/{}/ml_{}.csv'.format(DATA, DATA))
e_feat = np.load('./data/{}/ml_{}.npy'.format(DATA, DATA))
n_feat = np.load('./data/{}/ml_{}_node.npy'.format(DATA, DATA))
src_l = g_df.u.values
dst_l = g_df.i.values
...
if DATA in ['enron', 'socialevolve', 'uci', 'lastfm', 'copenhagen', 'wikipedia', 'USLegis', 'UNtrade']:
    node_zero_padding = np.zeros((n_feat.shape[0], 400 - n_feat.shape[1]))
    n_feat = np.concatenate([n_feat, node_zero_padding], axis=1)
    edge_zero_padding = np.zeros((e_feat.shape[0], 400 - e_feat.shape[1]))
...
if not CURRICULUM_LEARNING:
  ...
  train_rand_sampler = RandEdgeSampler_CLScheduler(train_src_l, 
                                                   train_dst_l, 
                                                   train_ts_l,
                                                   'hist_nre',
...
for i in range(N_RUNS):
    ...
    real_epoch = train_val(train_val_data, cawn, args.mode, BATCH_SIZE, NUM_EPOCH, criterion, optimizer, early_stopper, ngh_finders,
              rand_samplers, logger, real_epoch, CURRICULUM_LEARNING)
