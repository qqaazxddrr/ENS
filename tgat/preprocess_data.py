...
def preprocess(data_name, n_node_feat):
...
def run(data_name, bipartite=True, n_node_feat=172):
  Path("data/{}/".format(data_name)).mkdir(parents=True, exist_ok=True)
  ...
  df, feat = preprocess(PATH, n_node_feat)
...
parser = argparse.ArgumentParser('Interface for TGAT data preprocessing')
