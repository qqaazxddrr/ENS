...
from utils import EarlyStopMonitor, RandEdgeSampler, eval_one_epoch_modified, RandEdgeSampler_CLScheduler
...
parser.add_argument('--pacing', type=str, default="linear", choices=[
  "linear", "geometric", "root"], help='Type of pacing function of curriculum learning')
parser.add_argument('--CL_R', action='store_true', 
                    help='Whether to use reverse curriculum Learning, Use after the cl option takes effect ')
parser.add_argument('--NO_PROXIMITY', action='store_true', 
                    help='Whether to use node proximity in CL')
...
if DATA in ['enron', 'socialevolve', 'uci', 'lastfm', 'copenhagen', 'wikipedia', 'USLegis', 'UNtrade', 'Flights']:
...
if not CURRICULUM_LEARNING:
  ...
  train_rand_sampler = RandEdgeSampler_CLScheduler(train_data.sources, 
...
for i in range(args.n_runs):
    ...
    for epoch in range(NUM_EPOCH):
        ...
        for k in range(num_batch):
            ...
            neg_prob = tgan.contrast_modified(src_l_cut, negatives_batch, ts_l_cut, NUM_NEIGHBORS)
        ...
        def delete_checkpoint(epoch):
            ...
            filepath = get_checkpoint_path(epoch)
